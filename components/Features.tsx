    ...
    <section className="relative py-24 bg-white border-b border-gray-100 overflow-hidden">
      ...
      >
        <div className={`text-center mb-16 transition-all duration-700 ease-out transform ${
          ...
          <span className="inline-block py-1 px-3 rounded-full bg-blue-50 text-blue-600 text-xs font-bold tracking-wider uppercase mb-3 border border-blue-100">
            Core Features
          </span>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 tracking-tight">系统核心能力</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div 
              ...
              className={`group relative p-8 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
              }`}
            >
              {/* Bottom Accent Border */}
              <div className={`absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-gray-200 to-transparent transition-colors duration-300 ${feature.hoverBorder.replace('border-', 'via-')}`}></div>
